...
cpdef tste_grad(npX,
    ...
    cdef double K_AB, K_AC
    cdef double Q_triplet_AB, Q_triplet_AC
    cdef double K_triplet_AB, K_triplet_AC
    ...
    with nogil:
        ...
        for t in cython.parallel.prange(no_triplets):
            ...
            # Calculate values needed for the equation on the lower right of
            # t-STE paper page 3.
            # First calculate K_AB and K_AC as distance between AB and AC
            K_AB = sum_X[triplets_A[t]] + sum_X[triplets_B[t]]
            K_AC = sum_X[triplets_A[t]] + sum_X[triplets_C[t]]
            for k in xrange(no_dims):
                K_AB += -2 * X[triplets_A[t], k] * X[triplets_B[t], k]
                K_AC += -2 * X[triplets_A[t], k] * X[triplets_C[t], k]

            # Then calculate as described on page 3 lower right in t-STE paper
            K_AB = 1 + K_AB / alpha
            K_AC = 1 + K_AC / alpha
            ...
            K_triplet_AB = K_AB ** ((alpha + 1) / -2)
            K_triplet_AC = K_AC ** ((alpha + 1) / -2)

            P = K_triplet_AB / (K_triplet_AB + K_triplet_AC)
